...
class Neural2Player(BasePokerPlayer):
    ...
    def evaluate_network(self, data):
        ...
        n[3][3] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[50:55], n[2]))
        n[3][4] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[55:60], n[2]))
        
        # Forth layer
        n[4][0] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[60:65], n[3]))
        n[4][1] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[65:70], n[3]))
        n[4][2] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[70:75], n[3]))
        n[4][3] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[75:80], n[3]))
        n[4][4] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[80:85], n[3]))

        # Fifth layer
        n[5][0] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[85:90], n[4]))
        n[5][1] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[90:95], n[4]))
        n[5][2] = activation_functions.tanh(2.5, 0.5, float(2) / 2.5, 0.5)(numpy.dot(self.weights[95:100], n[4]))

        # Max
        return numpy.argmax(n[5])
